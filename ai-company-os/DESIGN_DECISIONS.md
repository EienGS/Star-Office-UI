...
- [x] **UI 详细设计**：平面图房间划分、楼层切换、Agent 动画、任务卡片交互（见第十六章）
...
完全沿用 Star Office UI 的现有像素风格，不重新设计：
...
| 标题色 | `#ffd700` |
| 次要背景 | `#3a3f4f` |
| 字体 | ArkPixel（像素字体），fallback: Courier New, monospace |
| 边框风格 | 2-4px solid，带 box-shadow |
...
┌──────────────────────────────────────────────────────────────────┐
│  顶部栏（40px）                                                    │
│  [公司名] [楼层切换 F1/F2/F3] [项目快速定位] [告警通知] [设置]    │
├─────────────────────────────────┬────────────────────────────────┤
│                                 │                                │
│  主视图：像素办公室平面图（Phaser.js）│  侧边面板（340px）           │
│  - Agent 像素角色 + 状态气泡     │  - 消息收件箱（CEO 视角）      │
│  - 房间区域交互                  │  - Agent 列表                  │
│  - Agent 走动动画               │  - 指令输入框                  │
│                                 │                                │
├─────────────────────────────────┴────────────────────────────────┤
│  底部：任务流水线泳道（200px）                                      │
│  [当前项目选择] [阶段1] → [阶段2] → [阶段3] → ... [自定义阶段]    │
└──────────────────────────────────────────────────────────────────┘
```

### 16.3 楼层概念

每个楼层是一个独立的 Phaser 场景，楼层切换时平滑过渡：

| 楼层 | 定位 | 内容 |
|------|------|------|
| **B1 - 基础设施层** | 公共服务 | 服务器室、数据库室、MCP 工具架 |
| **F1 - 公共层** | 日常协作 | 会议室、档案室、休息区、常驻 Agent 工位 |
| **F2+ - 项目层** | 项目工作 | 每个活跃项目一个楼层，Team Lead + Worker 工位 |

项目楼层动态创建：新建项目时系统自动分配一个楼层，项目归档后楼层保留为历史记录（可查看但标记为已完成）。

**楼层切换规则：**
- 顶部栏楼层按钮：F1 固定，F2/F3... 按项目数量动态显示
- 项目定位：下拉选择项目，系统自动跳转到对应楼层并高亮相关 Agent
- B1 固定入口：点击服务器图标或顶部栏 B1 按钮
...
│                                                         │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌────────┐  │
│  │ Agent A  │  │ Agent B  │  │ Agent C  │  │ 空工位 │  │
│  │ 工位     │  │ 工位     │  │ 工位     │  │(可分配)│  │
│  │ [状态气泡]│  │ [状态气泡]│  │ [状态气泡]│  │       │  │
│  └──────────┘  └──────────┘  └──────────┘  └────────┘  │
│                                                         │
│  ┌──────────────────────────────────────────────────┐   │
│  │  Worker 区（子 Task 在此显示，小像素角色）          │   │
│  └──────────────────────────────────────────────────┘   │
└────────────────────────────────────────────────────────┘
```
...
- **自动排列 + 可调整**：系统按创建顺序自动分配工位，CEO 可拖拽调整位置
- 拖拽位置持久化存储到 `/office/config/agents.json` 的 `uiPosition` 字段
- 项目楼层工位数超过一行时自动换行，不限制数量
...
|------|-----------|---------|
| idle | 站立/随机小动作 | 灰色 |
| working | 打字动画 | 绿色（`#00ff88`） |
| waiting | 等待动画（看表） | 黄色（`#ffd700`） |
| error | 感叹号动画 | 红色（`#e94560`） |

Agent 走动：跨房间操作时（如去会议室交接任务），像素角色播放走路动画移动到目标房间。

### 16.9 项目定位功能

顶部栏"项目定位"下拉框：
- 选择项目 → 跳转到对应楼层
- 高亮该项目所有关联 Agent 的工位（边框闪烁 `#e94560`）
- 底部流水线自动切换到该项目的阶段看板

### 16.10 侧边面板结构

三个 Tab 切换：
1. **消息**：CEO 收件箱（escalation + 回执通知），按时间倒序
2. **Agent**：所有 Agent 列表，点击定位到工位，支持按楼层/项目筛选
3. **指令**：CEO 指令输入框（完整输入能力，见第十五章）
